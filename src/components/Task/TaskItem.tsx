...
const TaskItem = ({ task, handleRemove, handleCheck }: Props) => {
    ...
    <div className="gap-6 mt-3 min-h-[72px] px-4 py-4 flex items-center bg-custom-gray-500 rounded-md">
      <label htmlFor="" className="relative w-6 h-6">
        ...
        <svg
          ...
          fill="none"
          onClick={() => {checkboxRef?.current?.click(); handleCheck(task.id)} }
          viewBox="0 0 24 24"
          stroke-width="1.5"
          stroke="white"
      ...
      <p className={task.status===true?"line-through text-custom-gray-300 text-sm font-inter flex-1" :"text-custom-gray-100 flex-1 text-sm font-inter"}>
        {task.title}
      </p>
